...
const budgetCollapseBillsButton = document.getElementById(
  "budget-collapse-bills"
);
const budgetExpandBillsButton = document.getElementById("budget-expand-bills");
...
const openPopover = (anchor, contentBuilder, { onClose } = {}) => {
  closeActivePopover();
...
const isDebitIndicator = (transaction) =>
  String(transaction?.creditDebitIndicator || "")
    .trim()
    .toLowerCase() === "debit";
  ...
  return {
    ...
    ui: {
      ...
      collapsedCategoryIds: Array.isArray(ui?.collapsedCategoryIds)
        ...
        : [],
      collapsedBillIds: Array.isArray(ui?.collapsedBillIds)
        ? ui.collapsedBillIds
        : [],
    },
  };
...
const normalizeBudgetTransactions = (state) => {
  state.transactions.forEach((transaction) => {
    ...
    if (!transaction.txKey) {
      const key = getTransactionKeyFromTransaction(transaction);
      if (key) {
        transaction.txKey = key;
      }
    }
    if (!transaction.tag) {
      if (
...
const updateActiveWorkspaceFromBudgetState = () => {
  ...
  activeWorkspace.ui = {
    ...
    collapsedBillIds: Array.from(collapsedBillIds),
  };
};
...
const applyWorkspaceState = (workspace) => {
  ...
  collapsedBillIds = new Set(workspace.ui?.collapsedBillIds || []);
...
const normalizeTransactionDescription = (value) => normalizeMatchText(value);

const formatTransactionKeyAmount = (amount) =>
  Number.isFinite(amount) ? Math.abs(amount).toFixed(2) : "";
...
const buildTransactionKey = ({
  ...
  const typeGroup = (transactionTypeGroup || transactionType || "")
    .trim()
    .toLowerCase();
  const indicator = String(creditDebitIndicator || "").trim().toLowerCase();
  const amountToken = formatTransactionKeyAmount(amount);
  return [
    postingDate || "",
    transactionDate || "",
    amountToken,
...
const getTransactionKeyFromTransaction = (transaction) =>
  buildTransactionKey({
    ...
    transactionTypeGroup: transaction.transactionTypeGroup || "",
...
const setBillCollapsed = (billId, isCollapsed) => {
  if (isCollapsed) {
    collapsedBillIds.add(billId);
  } else {
    collapsedBillIds.delete(billId);
  }
};
...
const recomputeBudgetDerivedState = () => {
  ...
  budgetState.transactions.forEach((transaction) => {
    ...
    if (isExcludedFromBudget(transaction)) {
      if (isDebitIndicator(transaction)) {
        excludedDebitTotal += getNormalizedTransactionAmount(transaction);
      }
      return;
    }
...
const renderBudgetTotals = () => {
  ...
  budgetTotals.innerHTML = [
    ...
    },
    {
      ...
      value: formatCurrency(derived.excludedDebitTotal),
      tooltip: "Sum of excluded debit transactions.",
    },
    {
...
const renderBudgetBills = () => {
  ...
  budgetState.bills.forEach((bill) => {
    ...
    group.className = "budget-bill-group";
    const isCollapsed = collapsedBillIds.has(bill.id);
    ...
    deleteButton.addEventListener("click", () => {
      ...
      collapsedBillIds.delete(bill.id);
      budgetState.transactions.forEach((transaction) => {
        if (
          transaction.tag?.type === "bill" &&
    ...
    if (toggleButton) {
      toggleButton.addEventListener("click", () => {
        setBillCollapsed(bill.id, !collapsedBillIds.has(bill.id));
        renderBudgetBills();
      });
    }
    group.appendChild(row);
    const billTransactions = budgetState.transactions.filter(
      (transaction) =>
        transaction.tag?.type === "bill" &&
        transaction.tag?.targetId === bill.id
    );
    if (billTransactions.length && !isCollapsed) {
      const transactionList = document.createElement("div");
      transactionList.className = "budget-bill-transactions";
      billTransactions.forEach((transaction) => {
        const transactionRow = document.createElement("div");
        transactionRow.className = "budget-row budget-row--bill-transaction";
    ...
    budgetBillsTable.appendChild(group);
  });
...
const renderBudgetUnintentional = () => {
  ...
  const assignOptions = hasAssignments
    ...
    : `
      ...
      ${excludeOption}
    `;
  const applyUnintentionalAssignment = (transaction, selection) => {
    ...
    .forEach((item) => {
      ...
      toggle.addEventListener("click", () => {
        ...
        openPopover(
          toggle,
          (popover) => {
            popover.classList.add("budget-unintentional__popover");
            popover.innerHTML = `
              <label class="budget-unintentional__assign-popover">
                <span>Assign to</span>
                <select data-role="unintentional-assign">
                  ${assignOptions}
                </select>
              </label>
            `;
            ...
            select.addEventListener("change", (event) => {
              ...
              applyUnintentionalAssignment(transaction, selection);
              closeActivePopover();
            });
            select.focus();
          },
...
const renderBudgetTransactionsSummary = () => {
  ...
  const billOptions = budgetState.bills.map((bill) => ({
    id: bill.id,
    name: bill.name,
  }));
  const categoryOptions = budgetState.categories.map((category) => ({
    id: category.id,
    name: category.name,
  }));
  ...
  filteredTransactions.forEach((transaction) => {
    ...
    const excludedLabel = transaction.excludedFromBudget
      ? '<span class="budget-excluded-label">Excluded</span>'
      : "";
    const summaryAssignOptions = `
      <option value="">${
        billOptions.length || categoryOptions.length
          ? "Assign to bill or category"
          : "No bills or categories"
      }</option>
      ${
        billOptions.length
          ? `<optgroup label="Bills">${billOptions
              .map(
                (bill) =>
                  `<option value="bill:${bill.id}" ${
                    assignmentType === "bill" && assignmentTarget === bill.id
                      ? "selected"
                      : ""
                  }>${bill.name}</option>`
              )
      ...
      ${
        categoryOptions.length
          ? `<optgroup label="Categories">${categoryOptions
              .map(
                (category) =>
                  `<option value="category:${category.id}" ${
                    assignmentType === "category" &&
                    assignmentTarget === category.id
                      ? "selected"
                      : ""
    ...
    row.innerHTML = `
      ...
      <div class="budget-summary__assign">
        <select data-role="summary-assign">
          ${summaryAssignOptions}
        </select>
        <button type="button" data-action="assign">Assign</button>
      </div>
      <button type="button" data-action="delete">Remove</button>
    `;
    const assignSelect = row.querySelector('[data-role="summary-assign"]');
    const assignButton = row.querySelector('[data-action="assign"]');
    const deleteButton = row.querySelector('[data-action="delete"]');
    if (assignSelect && assignButton) {
      const updateAssignButton = () => {
        assignButton.disabled = !assignSelect.value;
      };
      updateAssignButton();
      assignSelect.addEventListener("change", updateAssignButton);
      assignButton.addEventListener("click", () => {
        const selection = assignSelect.value;
        if (!selection) {
          return;
        }
        const [type, targetId] = selection.split(":");
...
const renderBudgetTransactions = () => {
  ...
  const filteredTransactions = budgetState.transactions.filter((transaction) => {
    if (isCreditOrTransferTransaction(transaction)) {
      return false;
    }
    if (budgetFilterMode === "reviewed") {
      if (!transaction.reviewed || transaction.excludedFromBudget) {
        return false;
      }
    }
    if (budgetFilterMode === "unreviewed") {
      if (transaction.reviewed || transaction.excludedFromBudget) {
        return false;
      }
    }
    if (budgetFilterMode === "excluded" && !transaction.excludedFromBudget) {
      return false;
    }
  ...
  filteredTransactions.forEach((transaction) => {
    ...
    row.innerHTML = `
      ...
      <div class="budget-transaction__footer">
        ...
        <div class="budget-transaction__actions">
          <button type="button" data-action="exclude">
            ${transaction.excludedFromBudget ? "Unexclude" : "Exclude"}
          </button>
    ...
    row.querySelector('[data-action="exclude"]').addEventListener("click", () => {
      transaction.excludedFromBudget = !transaction.excludedFromBudget;
      saveBudgetState();
      renderBudget();
    });
...
const importTransactionsFromCsv = (file) => {
  ...
  reader.onload = (event) => {
    ...
    const postingDateIndex = getIndex([
      "posting date",
      "posted date",
      "post date",
    ]);
    const transactionDateIndex = getIndex(["transaction date", "date"]);
    ...
    const typeGroupIndex = getIndex(["type group", "type group name"]);
    const categoryIndex = getIndex(["category"]);
    const transactionIndex = new Map();
    budgetState.transactions.forEach((transaction) => {
      const key = transaction.txKey || getTransactionKeyFromTransaction(transaction);
      if (key) {
        transaction.txKey = key;
        transactionIndex.set(key, transaction);
      }
    });
    let updatedCount = 0;
    const newTransactions = rows.slice(1).reduce((acc, row) => {
      const description = row[descriptionIndex] || "Transaction";
      const postingDate = row[postingDateIndex] || "";
      const transactionDate = row[transactionDateIndex] || "";
      const date = postingDate || transactionDate || "";
      ...
      const existing = transactionIndex.get(txKey);
      if (existing) {
        ...
        existing.transactionType = transactionType || null;
        existing.transactionTypeGroup = transactionTypeGroup;
        existing.category = category;
        existing.creditDebitIndicator = creditDebit || null;
        existing.txKey = txKey;
        updatedCount += 1;
        return acc;
      }
      ...
      const transaction = {
        ...
        transactionTypeGroup,
        category,
        creditDebitIndicator: creditDebit || null,
        txKey,
      ...
      transactionIndex.set(txKey, transaction);
      return acc;
    }, []);
    budgetState.transactions = [...newTransactions, ...budgetState.transactions];
    saveBudgetState();
    renderBudget();
    if (newTransactions.length || updatedCount) {
      setStatus(
        `Imported ${newTransactions.length} new transactions, updated ${updatedCount}.`
      );
    } else {
...
if (budgetCollapseBillsButton) {
  budgetCollapseBillsButton.addEventListener("click", () => {
    budgetState.bills.forEach((bill) => {
      collapsedBillIds.add(bill.id);
    });
    renderBudgetBills();
  });
}

if (budgetExpandBillsButton) {
  budgetExpandBillsButton.addEventListener("click", () => {
    collapsedBillIds.clear();
    renderBudgetBills();
  });
}
